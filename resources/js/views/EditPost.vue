<template>
    <section>
            ...
            <template slot="options">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        <router-link v-if="isPublished(post.published_at)" :to="{ name: 'post-stats', params: { id: uri } }"
                            class="dropdown-item">
                            {{ trans.view_stats }}
                        </router-link>
                        <div v-if="isPublished(post.published_at)" class="dropdown-divider" />
                        <a v-if="isDraft(post.published_at) && (isAdmin || isEditor)" href="#" class="dropdown-item"
                        ...
                        <a v-if="isPublished(post.published_at)" href="#" class="dropdown-item"
        ...
        <main v-if="isReady" class="py-4">
            <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-12">
                <div class="form-group my-3">
                    <textarea-autosize v-model="post.title" :placeholder="trans.title" style="font-size: 42px"
                        class="w-100 form-control-lg border-0 font-serif bg-transparent px-0" rows="1"
        ...
        <section v-if="isReady">
            ...
            <settings-modal ref="settingsModal" :post="post" :tags="tags" :topics="topics" :errors="errors"
                @sync-slug="updateSlug" @add-tag="addTag" @add-post-tag="addPostTag" @add-post-topic="addPostTopic"
                @add-topic="addTopic" @update-post="savePost" />
            <featured-image-modal ref="featuredImageModal" :post="post" @update-featured-image="updateFeaturedImage"
                @remove-featured-image="removeFeaturedImage" @update-post="savePost" />
            <seo-modal ref="seoModal" :post="post" @sync-title="updateMetaTitle" @sync-description="updateMetaDescription"
                @update-post="savePost" />
            <delete-modal ref="deleteModal" :header="trans.delete" :message="trans.deleted_posts_are_gone_forever"
                @delete="deletePost" />
        </section>
    </section>
</template>
...
import FeaturedImageModal from '../components/modals/FeaturedImageModal.vue';
