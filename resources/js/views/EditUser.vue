<template>
    <section>
        <page-header>
            <template slot="options">
                <div v-if="!creatingUser && !authProfile" class="dropdown">
                    <a id="navbarDropdown" class="nav-link pr-0" href="#" role="button" data-toggle="dropdown"
                        aria-haspopup="true" aria-expanded="false">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="25" class="icon-dots-horizontal">
                            <path class="fill-light-gray" fill-rule="evenodd"
        ...
        <main v-if="isReady" class="py-4">
            <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-12">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <select v-model="user.role" id="role" v-if="!creatingUser" :disabled="authProfile"
                        class="ml-auto w-auto custom-select border-0 bg-light" name="role" @change="selectRole">
                        <option :key="`${roleId}-${name}`" v-for="(name, roleId) in settings.roles" :value="roleId"
                    ...
                    <div class="card-body py-4">
                        <div class="row">
                            <div class="col-12 col-md-3 px-0 text-center">
                                <img :src="user.avatar || user.default_avatar" :alt="user.name"
                                    class="avatar align-self-center mb-4 mb-md-0 rounded-circle" @click="showAvatarModal" />
                            </div>
                        ...
                        <div class="col-12">
                            <div class="form-group row">
                                ...
                                <input v-model="user.name" id="name" name="name" type="text" required
                                    class="form-control border-0" :class="invalidName.shouldShow ? 'is-invalid' : ''"
                                    :title="trans.name" :placeholder="trans.name" />
                            ...
                            <div class="form-group row">
                                ...
                                <input v-model="user.email" id="email" required name="email" type="email"
                                    class="form-control border-0" :class="invalidEmail.shouldShow ? 'is-invalid' : ''"
                            ...
                            <div v-if="!creatingUser" class="form-group row">
                                ...
                                <input v-model="user.username" id="username" name="username" type="text"
                                    class="form-control border-0" :class="invalidUsername.shouldShow ? 'is-invalid' : ''"
                                    title="Username" :placeholder="trans.choose_a_username" />
                            ...
                            <div class="form-group row">
                                ...
                                <input v-model="user.password" id="password" :required="creatingUser" name="password"
                                    type="password" class="form-control border-0"
                                    :class="invalidPassword.shouldShow ? 'is-invalid' : ''" title="Password"
                            ...
                            <div class="form-group row">
                                ...
                                <input v-model="user.password_confirmation" id="password_confirmation"
                                    :required="creatingUser" name="password_confirmation" type="password"
                                    class="form-control border-0" title="Confirm Password" placeholder="Confirm Password" />
                            </div>
                            <div v-if="!creatingUser" class="form-group row">
                                <label for="summary" class="font-weight-bold text-uppercase text-muted small">
                                    {{ trans.summary }}
                                </label>
                                <textarea v-model="user.summary" id="summary" rows="4" name="summary" style="resize: none"
                                    class="form-control border-0" :placeholder="trans.tell_us_about_yourself" />
                            </div>

                            <div class="form-group row mt-4 mb-2">
                                <div class="col-md px-0">
                                    <a href="#" onclick="this.blur()" :disabled="shouldDisableSaveButton"
                                        class="btn btn-success btn-block font-weight-bold mt-0"
                                        :class="{ disabled: shouldDisableSaveButton }" aria-label="Save"
                    ...
                    <div class="card-body p-0">
                        <div :key="`${index}-${post.id}`" v-for="(post, index) in posts">
                            <router-link :to="{
                                ...
                                <div v-hover="{ class: `hover-bg` }" class="d-flex p-3 align-items-center" :class="{
                                    'border-top': index !== 0,
                                    'rounded-top': index === 0,
                                    ...
                                    <div class="ml-auto">
                                        <div class="d-none d-md-inline">
                                            <span class="text-secondary mr-3">{{ suffixedNumber(post.views_count) }}
                                                {{ post.views_count == 1 ? trans.view : trans.views }}</span>
                                            <span class="mr-3">{{ trans.created }}
        ...
        <section v-if="isReady">
            ...
            <delete-modal ref="deleteModal" :header="trans.delete" :message="trans.deleted_users_are_gone_forever"
                @delete="deleteUser" />
        </section>
    </section>
</template>
